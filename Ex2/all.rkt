...
[else (error 'parse-sexpr "bad syntax in ~s"
 ...
 ;;to a PLANG AST
 (define (parse str)
  ;;(parse-sexpr (string->sexpr str))) we can also just write this
  (let ([code (string->sexpr str)])
   (parse-sexpr code))) ;;we were asked to write this row
...
;;;;---------------------- Question 3.b.iii ------------------------------------------------
;; this was the most consusing one, i solved it somehow as we did in the lecture
...
;;we wre asked to implement the eval-poly function
;;im not sure about it !
(: eval-poly : PLANG -> <(Listof Number)> )
(define (eval-poly p-expr) ;;we wre asked to implement this function
   #|
  (cases expr
    [(Num n)   n]
    [(Add l r) (+ (eval l) (eval r))]
    ...
    [(Div l r) (/ (eval l) (eval r))])
   |#
 (local ((define power (reverse (build-list (length coeffs) values)))
         (define unknown-ls (build-list (length coeffs) (λ (i) x))))
   (foldr + 0 (map * coeffs (map expt unknown-ls power)))))
